    ...
    if(hasPreviousPage) {
        return (
            <Link className={"prev page-numbers"} to={previousLink}>
                ...
                <span className="nav-prev-text">Newer <span className="nav-short">Posts</span></span>
            </Link>
        )
    } else {
        return (
...
const renderPagesInBetween = ({hasPreviousPage, hasNextPage, currentPage, pageCount, archivePath}) => {
    ...
    return elementsToRender.map(({tag: Tag, children, ...props}, index) => <Tag key={index} {...props}>{children}</Tag>)
}
        ...
        <div className="pagination-wrapper section-inner">
            ...
            <nav className="navigation pagination" role="navigation" aria-label="Posts">
                ...
                <div className="nav-links">
                    ...
                    {/*<a className="page-numbers" href="/page/2/">2</a>*/}
